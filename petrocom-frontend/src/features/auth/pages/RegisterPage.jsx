      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8 animate-fade-in">
          ...
          <div className="flex flex-col items-center justify-center gap-2 min-h-[120px]">
            ...
            <p className="text-[#65647a] text-center max-w-xl">
        ...
        <div className="bg-[#f3f4f6] rounded-2xl shadow-xl p-8 animate-slide-up">
          {errors.general && (
            <div className="bg-[#f3f4f6] border-l-4 border-[#e8a12f] p-4 rounded-lg mb-6 animate-fade-in flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-[#e8a12f] flex-shrink-0 mt-0.5" />
              <p className="text-[#e8a12f] text-sm font-medium">{errors.general}</p>
            </div>
          )}
          <form onSubmit={handleNext} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full bg-[#1fb74d] hover:bg-[#168a3d] text-white font-bold py-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
            >
              Continuar
            </button>
          </form>
          ...
          <div className="mt-8 text-center">
            <p className="text-[#65647a]">
              ...
              <Link to="/login" className="text-[#1fb74d] hover:text-[#e8a12f] font-bold transition-colors">
                Inicia sesión
              </Link>
            </p>
          </div>
...
const InputField = ({
  ...
  <div className="relative group">
    ...
    <div
      className={`flex items-center px-4 py-3 border-2 rounded-xl transition-all duration-300 bg-white shadow-sm focus-within:border-[#1fb74d] focus-within:ring-2 focus-within:ring-[#1fb74d]/20 ${
        error ? 'border-[#e8a12f]' : 'border-[#dfe2ea]'
      }`}
    >
      {Icon && <Icon className={`w-5 h-5 mr-3 ${focusedField === name ? 'text-[#1fb74d]' : 'text-[#65647a]'}`} />}
      <input
        type={type}
        name={name}
        value={value}
        ...
        maxLength={maxLength}
        className="w-full bg-transparent focus:outline-none text-[#07073b] placeholder:text-[#65647a]"
      />
    </div>
    {error && (
...
const PasswordField = ({ label, name, value, onChange, error, show, onToggle, focusedField, setFocusedField, placeholder }) => (
  <div className="relative group">
    <label className="block text-sm font-semibold text-[#07073b] mb-2">{label}</label>
    <div
      className={`relative px-4 py-3 border-2 rounded-xl transition-all duration-300 bg-white shadow-sm focus-within:border-[#1fb74d] focus-within:ring-2 focus-within:ring-[#1fb74d]/20 ${
        error ? 'border-[#e8a12f]' : 'border-[#dfe2ea]'
      }`}
    >
      <Lock className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 ${focusedField === name ? 'text-[#1fb74d]' : 'text-[#65647a]'}`} />
      <input
        ...
        onBlur={() => setFocusedField('')}
        placeholder={placeholder}
        className="w-full bg-transparent focus:outline-none text-[#07073b] placeholder:text-[#65647a] pl-12 pr-12"
      />
      <button
        type="button"
        onClick={onToggle}
        className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center text-[#65647a] hover:text-[#1fb74d] transition-colors"
      >
    ...
    <div className="min-h-screen bg-[#f3f4f6] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8 animate-fade-in">
          ...
          <p className="text-[#65647a]">Revisa y acepta nuestros términos para continuar</p>
        </div>
        ...
        <div className="bg-[#f3f4f6] rounded-2xl shadow-xl p-8 animate-slide-up">
          {(localError || apiError) && (
            <div className="bg-[#f3f4f6] border-l-4 border-[#e8a12f] p-4 rounded-lg mb-6 animate-fade-in flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-[#e8a12f] flex-shrink-0 mt-0.5" />
              <p className="text-[#e8a12f] text-sm font-medium">{localError || apiError}</p>
            </div>
          )}

          <div className="border border-[#dfe2ea] rounded-xl p-6 max-h-64 overflow-y-auto mb-6">
            <div className="space-y-4 text-sm text-[#65647a]">
          ...
          <label className="flex items-start gap-3 p-4 rounded-xl border-2 border-[#dfe2ea] hover:border-[#1fb74d] cursor-pointer transition-all mb-6">
            <input
              type="checkbox"
              checked={accepted}
              onChange={(e) => setAccepted(e.target.checked)}
              className="w-5 h-5 mt-0.5 text-[#1fb74d] focus:ring-[#1fb74d] rounded"
            />
          ...
          <div className="flex gap-4">
            <button
              ...
              onClick={onBack}
              disabled={loading}
              className="flex-1 border-2 border-[#dfe2ea] text-[#07073b] font-bold py-4 rounded-xl hover:bg-[#f3f4f6] transition-all disabled:opacity-50"
            >
              Atrás
            </button>
            <button
              ...
              className="flex-1 bg-[#1fb74d] hover:bg-[#168a3d] text-white font-bold py-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
