    ...
    <div className="min-h-screen bg-[#f3f4f6] py-8">
      <div className="container-custom max-w-7xl space-y-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#65647a]">Centro de notificaciones</p>
        ...
        <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_360px]">
          <div className="space-y-4">
            <div className="flex flex-col gap-3 rounded-lg border border-[#dfe2ea] bg-white p-4 shadow-sm md:flex-row md:items-center">
              ...
              <select
                ...
                className="rounded-lg border border-[#dfe2ea] bg-white px-3 py-2 text-[#07073b] outline-none"
              >
              ...
              <span className="ml-auto text-sm font-semibold text-[#65647a]">
                {unreadCount} no leida(s)
              </span>
            </div>

            <div className="overflow-hidden rounded-lg border border-[#dfe2ea] bg-white shadow-sm">
              {loading ? (
                <div className="p-8 text-center text-[#65647a]">Cargando notificaciones...</div>
              ) : items.length === 0 ? (
                <div className="flex flex-col items-center gap-3 p-12 text-center text-[#65647a]">
                ...
                <div className="divide-y divide-[#f3f4f6]">
                  {items.map((notification) => {
                    ...
                    const tone = typeTone[notification.type] || 'border-[#dfe2ea] bg-[#f3f4f6] text-[#07073b]';
                    ...
                    return (
                      <button
                        ...
                        className={`block w-full px-5 py-4 text-left transition hover:bg-[#ffffff] ${unread ? 'bg-[#fbf3df]' : 'bg-white'}`}
                      >
                        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                          <div className="min-w-0 space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {unread && <span className="rounded-full bg-[#1fb74d] px-2 py-0.5 text-xs font-bold text-white">Nueva</span>}
                            </div>
                            ...
                            <p className="text-sm leading-6 text-[#65647a]">{notification.data?.message}</p>
                            <p className="text-xs font-semibold text-[#65647a]">{notification.data?.tramite_code}</p>
                          </div>
                          <span className="shrink-0 text-xs text-[#65647a]">
          ...
          <aside className="rounded-lg border border-[#dfe2ea] bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              <Settings className="h-5 w-5 text-[#1fb74d]" />
            ...
            <div className="space-y-2">
              {preferences.map((item) => (
                <button
                  ...
                  className="flex w-full items-center justify-between gap-3 rounded-lg border border-[#dfe2ea] px-3 py-2 text-left transition hover:bg-[#f3f4f6] disabled:opacity-60"
                >
                  <span className="text-sm font-semibold text-[#07073b]">{item.label}</span>
                  {item.enabled ? (
                    <ToggleRight className="h-6 w-6 shrink-0 text-green-600" />
                  ) : (
                    <ToggleLeft className="h-6 w-6 shrink-0 text-[#65647a]" />
                  )}
                </button>
              ))}
            </div>
